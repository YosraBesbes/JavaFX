...
public interface InvoicingRepository extends CrudRepository<Invoicing, Integer> {
    ...
    @Query("    select new ph.txtdis.model.ProductivityPerFamilyByRoute(f, "
            ...
            + "  where s.orderDate between ?1 and ?2 and r.name = 'PMS1' group by i.family), "
            + "(select count(distinct s.partner) from Invoicing s join s.details d join d.item i join s.route r "
            + "  where s.orderDate between ?1 and ?2 and r.name = 'PMS2' group by i.family), "
            + "(select count(distinct s.partner) from Invoicing s join s.details d join d.item i join s.route r "
            + "  where s.orderDate between ?1 and ?2 and r.name = 'PMS3' group by i.family), "
            + "(select count(distinct s.partner) from Invoicing s join s.details d join d.item i join s.route r "
            + "  where s.orderDate between ?1 and ?2 and r.name = 'S41' group by i.family), "
            + "(select count(distinct s.partner) from Invoicing s join s.details d join d.item i join s.route r "
            + "  where s.orderDate between ?1 and ?2 and r.name = 'S42' group by i.family), "
            + "(select count(distinct s.partner) from Invoicing s join s.details d join d.item i join s.route r "
            + "  where s.orderDate between ?1 and ?2 and r.name = 'S43' group by i.family), "
            + "(select count(distinct s.partner) from Invoicing s join s.details d join d.item i join s.route r "
            + "  where s.orderDate between ?1 and ?2 and r.name = 'S44' group by i.family), "
            + "(select count(distinct s.partner) from Invoicing s join s.details d join d.item i join s.route r "
            + "  where s.orderDate between ?1 and ?2 and r.name = 'S45' group by i.family), "
            + "(select count(distinct s.partner) from Invoicing s join s.details d join d.item i join s.route r "
            + "  where s.orderDate between ?1 and ?2 and r.name = 'S46' group by i.family), "
            + "(select count(distinct s.partner) from Invoicing s join s.details d join d.item i join s.route r "
            + "  where s.orderDate between ?1 and ?2 and r.name = 'S47' group by i.family), "
            + "(select count(distinct s.partner) from Invoicing s join s.details d join d.item i join s.route r "
            + "  where s.orderDate between ?1 and ?2 and r.name = 'S48' group by i.family), "
            + "(select count(distinct s.partner) from Invoicing s join s.details d join d.item i join s.route r "
            + "  where s.orderDate between ?1 and ?2 and r.name = 'S49' group by i.family)) "
    ...
    @Query("  select new ph.txtdis.model.VolumePerChannelByRoute(c, "
            ...
            + "    and s.name = 'S48'), "
            + "(select sum(s.qty) from InvoicedVolumePerChannelByRouteView s where s.orderDate between ?1 and ?2 "
            + "    and s.name = 'S49')) from Channel c order by c.id ")
