...
public class LoanBoxedTable extends AbstractBoxedTable<Loan, EmployeeDTO> {
	...
	protected void addTableColumns() {
		...
		TableColumn<Loan, LoanType> typeCol = FX.addComboColumn("Type", "type",
				LoanType.values());
		TableColumn<Loan, Date> startCol = FX.addDateColumn("Start",
				"startDate");
		TableColumn<Loan, BigDecimal> amountCol = FX.addPriceColumn("Amount",
				"amount");
		...
		TableColumn<Loan, BigDecimal> paymentCol = FX.createColumn("Payment",
				"payment", 100);
		paymentCol.setCellFactory(column -> {
			return new TabledPriceFieldTableCell<Loan, Payment>(stage) {
				@Override
				protected Loan addItems(Loan loan,
						Tabled<Payment, Loan> dialog) {
					loan.setPayments(dialog.getAddedItems(loan));
					return loan;
				}

				@Override
				protected Tabled<Payment, Loan> addDialog(
						Stage stage, Loan loan) {
		...
		TableColumn<Loan, BigDecimal> balanceCol = FX.addPriceColumn("Balance",
				"balance");
		balanceCol.setEditable(false);

		table.getColumns().addAll(typeCol, startCol, amountCol, paymentCol,
				balanceCol);
	}
